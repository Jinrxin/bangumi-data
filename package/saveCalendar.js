  ...
  await Promise.all(
    calendarItem.items.map(async (anime) => {
      ...
      try {
        ...
        anime.nextEpisode =
          latestEpisode?.ep < episodes.data.length ? latestEpisode?.ep + 1 : episodes.data.length;
      } catch (error) {
        console.log(`ID:${anime.id}episodes信息不存在`);
      }
    })
  );
};
...
const saveCalendar = async () => {
  try {
    ...
    // 保存动漫详细信息
    await saveSubjectList(subjectIds, 500, 1, data.items);
